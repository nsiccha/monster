functions {
  vector constrain_unit_log_vector(vector y){
    vector[rows(y)] rv = y;
    rv[5] = -sum(rv[2:4]);
    rv[7] = -rv[6];
    return rv;
...
transformed parameters {
  vector[no_latent_params] constrained_unit_log_population_eM = constrain_unit_log_vector(
  ...
  for(person in 1:no_persons){
    constrained_unit_log_person_params[person] = constrain_unit_log_vector(
      unit_log_person_params[person]
    );
    // constrained_unit_log_person_params[person,5] = -sum(unit_log_person_params[person, 2:4]);
    // constrained_unit_log_person_params[person,7] = -unit_log_person_params[person, 6];
    log_person_params[person] = constrain_log_vector(
      log_population_eM + log_population_eS .* constrained_unit_log_person_params[person]
    );
    // log_person_params[person, 2:5] = log_softmax(log_person_params[person, 2:5]);
    // log_person_params[person, 6:7] = log_softmax(log_person_params[person, 6:7]);
...
model {
  for(person in 1:no_persons){
    target += normal_lpdf(constrained_unit_log_person_params[person] | 0,1);
    target += normal_lpdf(unit_log_person_params[person, 5] | 0,1);
    target += normal_lpdf(unit_log_person_params[person, 7] | 0,1);
  }
  target += normal_lpdf(constrained_unit_log_population_eM | 0,1);
  target += normal_lpdf(unit_log_population_eM[5] | 0,1);
  target += normal_lpdf(unit_log_population_eM[7] | 0,1);
  target += scaled_inv_chi_square_lpdf(
    pow(log_population_eS, 2) | population_eS_nu + (likelihood == 0 ? 1 : 0), log_population_eS_mu
  );
