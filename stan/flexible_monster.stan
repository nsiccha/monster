functions {
  vector prepare_unit_log_vector(vector y, int enforce_constraints){
    if(!enforce_constraints){return y;}
    vector[rows(y)] rv = y;
    rv[5] = -sum(rv[2:4]);
    rv[7] = -rv[6];
    return rv;
  }
  vector constrain_log_vector(vector y, int enforce_constraints){
    if(!enforce_constraints){return y;}
    vector[rows(y)] rv = y;
...
transformed parameters {
  vector[no_latent_params] constrained_unit_log_population_eM = prepare_unit_log_vector(
    unit_log_population_eM, enforce_constraints
  );
  ...
  if(!enforce_constraints){
    ...
    population_eM[6:7] = population_eM[6:7] / sum(population_eM[6:7]);
  }
  population_eM[6:7] = (
    .837 - population_eM[8]
  ) * population_eM[6:7];
  for(person in 1:no_persons){
    constrained_unit_log_person_params[person] = prepare_unit_log_vector(
      unit_log_person_params[person], enforce_constraints
    );
    ...
    if(!enforce_constraints){
      ...
      person_params[person, 6:7] = person_params[person, 6:7] / sum(person_params[person, 6:7]);
    }
